...
export default async function handler(req, res) {
    ...
    try {
        ...
        // **Aquí debes pegar el resto de la lógica de tu API para procesar los eventos.**
        // Si necesitas un ejemplo simple, usa el siguiente:
        const events = await eventsCollection.find({}).limit(10).toArray();
        res.status(200).json(events);

    } catch (err) {
        // Aquí es donde capturamos el error
